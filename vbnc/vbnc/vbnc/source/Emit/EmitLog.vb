...
#End If
    ReadOnly Property LocalType() As Type
        Get
            Return SRELocal.LocalType
        End Get
    End Property
End Class
...
#If ENABLECECIL Then
    ...
    Private Sub BeginCatchBlock(ByVal exceptionType As Mono.Cecil.TypeReference)
        Log("BeginCatchBlock")
    ...
    Private Function DeclareLocal(ByVal localType As Mono.Cecil.TypeReference) As Mono.Cecil.Cil.VariableDefinition
    ...
    Private Function DeclareLocal(ByVal localType As Mono.Cecil.TypeReference, ByVal pinned As Boolean) As Mono.Cecil.Cil.VariableDefinition
        Helper.Assert(Helper.IsEmittableMember(Compiler, localType))
        Log("DeclareLocal({0})", localType.ToString)
    ...
    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode)
        Log("Emit({0})", opcode.ToString)
        CilWorker.Emit(opcode)
    End Sub

    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal arg As Byte)
        Log("Emit({0},{1})", opcode.ToString, ToString(arg))
        CilWorker.Emit(opcode, arg)
    End Sub

    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal arg As Double)
        Log("Emit({0},{1})", opcode.ToString, ToString(arg))
        CilWorker.Emit(opcode, arg)
    End Sub

    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal arg As Integer)
        Log("Emit({0},{1})", opcode.ToString, ToString(arg))
        CilWorker.Emit(opcode, arg)
    End Sub

    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal arg As Long)
        Log("Emit({0},{1})", opcode.ToString, ToString(arg))
        CilWorker.Emit(opcode, arg)
    End Sub

    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal arg As SByte)
        Log("Emit({0},{1})", opcode.ToString, ToString(arg))
        CilWorker.Emit(opcode, arg)
    End Sub

    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal arg As Short)
        Log("Emit({0},{1})", opcode.ToString, ToString(arg))
        CilWorker.Emit(opcode, arg)
    End Sub

    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal arg As Single)
        Log("Emit({0},{1})", opcode.ToString, ToString(arg))
        CilWorker.Emit(opcode, arg)
    End Sub
    ...
    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal label As Mono.Cecil.Cil.Instruction)
        Log("Emit({0},{1})", opcode.ToString, ToString(label))
        CilWorker.Emit(opcode, label)
    End Sub

    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal labels() As Mono.Cecil.Cil.Instruction)
        Log("Emit({0},{1})", opcode.ToString, ToString(labels))
        CilWorker.Emit(opcode, labels)
    End Sub

    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal local As Mono.Cecil.Cil.VariableDefinition)
        Log("Emit({0},{1})", opcode.ToString, ToString(local))
        CilWorker.Emit(opcode, local)
    End Sub

    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal signature As System.Reflection.Emit.SignatureHelper)
        Log("Emit({0},{1})", opcode.ToString, ToString(signature))
    ...
    Private Sub Emit(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal cls As Mono.Cecil.TypeReference)
        Helper.Assert(Helper.IsEmittableMember(Compiler, cls))
        Log("Emit({0},{1})", opcode.ToString, ToString(cls))
        CilWorker.Emit(opcode, cls)
    End Sub

    Private Sub EmitCall(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal methodInfo As Mono.Cecil.MethodReference, ByVal optionalParameterTypes() As Mono.Cecil.TypeReference)
    ...
    Private Sub EmitCalli(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal callingConvention As System.Reflection.CallingConventions, ByVal returnType As System.Type, ByVal parameterTypes() As System.Type, ByVal optionalParameterTypes() As System.Type)
        Helper.Assert(Helper.IsEmittableMember(returnType))
        Helper.Assert(Helper.IsEmittableMember(parameterTypes))
        Helper.Assert(Helper.IsEmittableMember(optionalParameterTypes))
        Log("EmitCalli({0},{1},{2},{3},{4})", opcode.ToString, ToString(callingConvention), ToString(returnType), ToString(parameterTypes), ToString(optionalParameterTypes))
    ...
    Private Sub EmitCalli(ByVal opcode As Mono.Cecil.Cil.OpCode, ByVal unmanagedCallConv As System.Runtime.InteropServices.CallingConvention, ByVal returnType As System.Type, ByVal parameterTypes() As System.Type)
        Helper.Assert(Helper.IsEmittableMember(returnType))
        Helper.Assert(Helper.IsEmittableMember(parameterTypes))
        Log("EmitCalli({0},{1},{2},{3})", opcode.ToString, ToString(unmanagedCallConv), ToString(returnType), ToString(parameterTypes))
    ...
    Private Sub EmitWriteLine(ByVal localBuilder As Mono.Cecil.Cil.Instruction)
        Log("EmitWriteLine({0})", ToString(localBuilder))
        Throw New NotImplementedException("m_ILGen.EmitWriteLine(localBuilder)")
    End Sub

    Private Sub EmitWriteLine(ByVal fld As Mono.Cecil.FieldReference)
        Helper.Assert(Helper.IsEmittableMember(Compiler, fld))
        Log("EmitWriteLine({0})", ToString(fld))
        Throw New NotImplementedException("m_ILGen.EmitWriteLine(fld)")
    End Sub

    Private Sub EndScopeCecil()
        Log("EndScope")
        m_ILGen.EndScope()
    End Sub

    Private Sub MarkLabel(ByVal loc As Mono.Cecil.Cil.Instruction)
        Log("MarkLabel({0}", ToString(loc))
    ...
    Private Sub ThrowException(ByVal excType As Mono.Cecil.TypeReference)
        Helper.Assert(Helper.IsEmittableMember(Compiler, excType))
        Log("ThrowException({0})", ToString(excType))
        CilWorker.Emit(Mono.Cecil.Cil.OpCodes.Throw, excType)
    End Sub

    Private Sub UsingNamespaceCecil(ByVal usingNamespace As String)
        Log("UsingNamespace({0})", usingNamespace)
